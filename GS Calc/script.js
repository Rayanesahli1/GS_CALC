...
function drawCrossSection() {
  ...
  const svgContainer = document.getElementById('sectionSVG');
  const dimsContainer = document.getElementById('sectionDims');
  ...
  const dimLineColor = isDark ? 'rgba(148,163,184,0.4)' : 'rgba(100,116,139,0.35)';

  const isUPN = profile.startsWith('UPN');
  ...
  // SVG canvas sizing — scale to fit nicely
  const marginLeft = 55;
  ...
  const maxDrawH = 180;
  const maxDrawW = 160;
  const scale = Math.min(maxDrawH / h, maxDrawW / b);

  const drawH = h * scale;
  const drawB = b * scale;
  const drawTf = Math.max(tf * scale, 3);
  const drawTw = Math.max(tw * scale, 3);

  const svgW = drawB + marginLeft + marginRight + 60;
  const svgH = drawH + marginTop + marginBottom + 10;

  // Origin = top-left of the cross-section bounding box
  const ox = marginLeft;
  const oy = marginTop;
  ...
  if (isUPN) {
    // U-channel (open to the right)
    // Bottom flange: full width at bottom
    // Top flange: full width at top
    // Web: on the left side
    shapePath = [
      `M ${ox} ${oy}`,                                    // top-left
      `L ${ox + drawB} ${oy}`,                              // top-right
      `L ${ox + drawB} ${oy + drawTf}`,                     // inner top-right
      `L ${ox + drawTw} ${oy + drawTf}`,                    // inner top-left (web)
      `L ${ox + drawTw} ${oy + drawH - drawTf}`,            // inner bottom-left (web)
      `L ${ox + drawB} ${oy + drawH - drawTf}`,             // inner bottom-right
      `L ${ox + drawB} ${oy + drawH}`,                      // bottom-right
      `L ${ox} ${oy + drawH}`,                              // bottom-left
      'Z'
    ].join(' ');
  } else {
    // I-shaped section (IPE, HEA, HEB, IPN, HEM)
    const flangeOffset = (drawB - drawTw) / 2;
    shapePath = [
      `M ${ox} ${oy}`,                                        // top-left
      `L ${ox + drawB} ${oy}`,                                  // top-right
      `L ${ox + drawB} ${oy + drawTf}`,                         // outer top-right flange bottom
      `L ${ox + drawB / 2 + drawTw / 2} ${oy + drawTf}`,       // inner right of web top
      `L ${ox + drawB / 2 + drawTw / 2} ${oy + drawH - drawTf}`, // inner right of web bottom
      `L ${ox + drawB} ${oy + drawH - drawTf}`,                 // outer bottom-right flange top
      `L ${ox + drawB} ${oy + drawH}`,                          // bottom-right
      `L ${ox} ${oy + drawH}`,                                  // bottom-left
      `L ${ox} ${oy + drawH - drawTf}`,                         // outer bottom-left flange top
      `L ${ox + drawB / 2 - drawTw / 2} ${oy + drawH - drawTf}`, // inner left of web bottom
      `L ${ox + drawB / 2 - drawTw / 2} ${oy + drawTf}`,         // inner left of web top
      `L ${ox} ${oy + drawTf}`,                                  // outer top-left flange bottom
      'Z'
    ].join(' ');
  }
  ...
  // --- h dimension (left side, vertical) ---
  const hx = ox - 30;
  dims += `<line x1="${hx}" y1="${oy}" x2="${hx}" y2="${oy + drawH}" stroke="${dimLineColor}" stroke-width="1"/>`;
  // Arrows
  dims += `<polygon points="${hx},${oy} ${hx - arrowSize},${oy + arrowSize * 2} ${hx + arrowSize},${oy + arrowSize * 2}" fill="${dimColor}"/>`;
  dims += `<polygon points="${hx},${oy + drawH} ${hx - arrowSize},${oy + drawH - arrowSize * 2} ${hx + arrowSize},${oy + drawH - arrowSize * 2}" fill="${dimColor}"/>`;
  // Leader lines
  dims += `<line x1="${ox - 6}" y1="${oy}" x2="${hx - 8}" y2="${oy}" stroke="${dimLineColor}" stroke-width="0.7" stroke-dasharray="3,2"/>`;
  dims += `<line x1="${ox - 6}" y1="${oy + drawH}" x2="${hx - 8}" y2="${oy + drawH}" stroke="${dimLineColor}" stroke-width="0.7" stroke-dasharray="3,2"/>`;
  // Label
  dims += `<text x="${hx - 4}" y="${oy + drawH / 2 + 4}" font-size="${fontSize}" fill="${accentColor}" font-weight="600" text-anchor="end" font-family="Inter, sans-serif">h=${h}</text>`;

  // --- b dimension (bottom, horizontal) ---
  const by = oy + drawH + 20;
  dims += `<line x1="${ox}" y1="${by}" x2="${ox + drawB}" y2="${by}" stroke="${dimLineColor}" stroke-width="1"/>`;
  // Arrows
  dims += `<polygon points="${ox},${by} ${ox + arrowSize * 2},${by - arrowSize} ${ox + arrowSize * 2},${by + arrowSize}" fill="${dimColor}"/>`;
  dims += `<polygon points="${ox + drawB},${by} ${ox + drawB - arrowSize * 2},${by - arrowSize} ${ox + drawB - arrowSize * 2},${by + arrowSize}" fill="${dimColor}"/>`;
  // Leader lines
  dims += `<line x1="${ox}" y1="${oy + drawH + 4}" x2="${ox}" y2="${by + 4}" stroke="${dimLineColor}" stroke-width="0.7" stroke-dasharray="3,2"/>`;
  dims += `<line x1="${ox + drawB}" y1="${oy + drawH + 4}" x2="${ox + drawB}" y2="${by + 4}" stroke="${dimLineColor}" stroke-width="0.7" stroke-dasharray="3,2"/>`;
  // Label
  dims += `<text x="${ox + drawB / 2}" y="${by + 15}" font-size="${fontSize}" fill="${accentColor}" font-weight="600" text-anchor="middle" font-family="Inter, sans-serif">b=${b}</text>`;

  // --- tf dimension (right side, at top flange) ---
  const tfx = ox + drawB + 15;
  dims += `<line x1="${tfx}" y1="${oy}" x2="${tfx}" y2="${oy + drawTf}" stroke="${dimLineColor}" stroke-width="1"/>`;
  dims += `<polygon points="${tfx},${oy} ${tfx - arrowSize},${oy + arrowSize * 1.5} ${tfx + arrowSize},${oy + arrowSize * 1.5}" fill="${dimColor}"/>`;
  dims += `<polygon points="${tfx},${oy + drawTf} ${tfx - arrowSize},${oy + drawTf - arrowSize * 1.5} ${tfx + arrowSize},${oy + drawTf - arrowSize * 1.5}" fill="${dimColor}"/>`;
  // Leader lines
  dims += `<line x1="${ox + drawB + 4}" y1="${oy}" x2="${tfx + 6}" y2="${oy}" stroke="${dimLineColor}" stroke-width="0.7" stroke-dasharray="3,2"/>`;
  dims += `<line x1="${ox + drawB + 4}" y1="${oy + drawTf}" x2="${tfx + 6}" y2="${oy + drawTf}" stroke="${dimLineColor}" stroke-width="0.7" stroke-dasharray="3,2"/>`;
  dims += `<text x="${tfx + 10}" y="${oy + drawTf / 2 + 4}" font-size="${smallFont}" fill="${dimColor}" font-weight="500" font-family="Inter, sans-serif">tf=${tf}</text>`;

  // --- tw dimension (center, horizontal across web) ---
  if (!isUPN) {
    const twy = oy + drawH / 2;
    const twLeft = ox + drawB / 2 - drawTw / 2;
    const twRight = ox + drawB / 2 + drawTw / 2;
    dims += `<line x1="${twLeft}" y1="${twy}" x2="${twRight}" y2="${twy}" stroke="${dimLineColor}" stroke-width="1"/>`;
    dims += `<polygon points="${twLeft},${twy} ${twLeft + arrowSize * 1.5},${twy - arrowSize} ${twLeft + arrowSize * 1.5},${twy + arrowSize}" fill="${dimColor}"/>`;
    dims += `<polygon points="${twRight},${twy} ${twRight - arrowSize * 1.5},${twy - arrowSize} ${twRight - arrowSize * 1.5},${twy + arrowSize}" fill="${dimColor}"/>`;
    dims += `<text x="${ox + drawB / 2}" y="${twy - 8}" font-size="${smallFont}" fill="${dimColor}" font-weight="500" text-anchor="middle" font-family="Inter, sans-serif">tw=${tw}</text>`;
  } else {
    // For UPN, tw is at the left web
    const twy = oy + drawH / 2;
    dims += `<line x1="${ox}" y1="${twy}" x2="${ox + drawTw}" y2="${twy}" stroke="${dimLineColor}" stroke-width="1"/>`;
    dims += `<text x="${ox + drawTw / 2}" y="${twy - 8}" font-size="${smallFont}" fill="${dimColor}" font-weight="500" text-anchor="middle" font-family="Inter, sans-serif">tw=${tw}</text>`;
  }

  // Build final SVG
  const svg = `<svg viewBox="0 0 ${svgW} ${svgH + 20}" width="100%" height="100%" style="max-width:${svgW}px;max-height:${svgH + 20}px;" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="sectionGrad" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="${isDark ? '#334155' : '#cbd5e1'}" />
        <stop offset="100%" stop-color="${isDark ? '#1e293b' : '#e2e8f0'}" />
      </linearGradient>
    </defs>
    <path d="${shapePath}" fill="url(#sectionGrad)" stroke="${strokeColor}" stroke-width="1.8" stroke-linejoin="round" />
    ${dims}
  </svg>`;
  ...
  // Dimension pills
  if (dimsContainer) {
    const pillStyle = `display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;font-size:12px;font-weight:500;font-family:Inter,sans-serif;`;
    const bgPill = isDark ? 'background:rgba(99,102,241,0.12);color:#a5b4fc;' : 'background:rgba(99,102,241,0.08);color:#4f46e5;';
    const bgPill2 = isDark ? 'background:rgba(148,163,184,0.12);color:#94a3b8;' : 'background:rgba(100,116,139,0.08);color:#475569;';
    dimsContainer.innerHTML = `
      <span style="${pillStyle}${bgPill}">h = ${h} mm</span>
      <span style="${pillStyle}${bgPill}">b = ${b} mm</span>
      <span style="${pillStyle}${bgPill2}">tf = ${tf} mm</span>
      <span style="${pillStyle}${bgPill2}">tw = ${tw} mm</span>
      <span style="${pillStyle}${bgPill2}">Iz = ${formatNumber(data.Iz, 1)} cm⁴</span>
      <span style="${pillStyle}${bgPill2}">It = ${formatNumber(data.It, 1)} cm⁴</span>
      <span style="${pillStyle}${bgPill2}">Iw = ${formatNumber(data.Iw, 0)} cm⁶</span>
      <span style="${pillStyle}${bgPill2}">Wpl = ${formatNumber(data.Wpl, 0)} cm³</span>
    `;
  }
}
...
function selectLoadType(value, btn) {
  // Update the hidden select
  document.getElementById('loading').value = value;
  // Update active card UI
  document.querySelectorAll('.load-type-card').forEach(c => c.classList.remove('active'));
...
function getTorsionWarping() {
  const el = document.getElementById('torsionWarping');
  // Strip letter suffixes like 'a','b' used to uniquify option values (e.g. "0.5_0.5a" → "0.5_0.5")
  const rawVal = el ? el.value.replace(/[a-z]$/i, '') : '1.0_1.0';
  const val = rawVal.split('_');
  return { kz: parseFloat(val[0]) || 1.0, kw: parseFloat(val[1]) || 1.0 };
}

function selectTW(value, btn) {
  document.getElementById('torsionWarping').value = value;
  document.querySelectorAll('.tw-card').forEach(c => c.classList.remove('active'));
  btn.classList.add('active');
  calculate();
}

function calculateMcrParams(L, support, zg, zj, kwUnused, data) {
  const E = 210000, G = 80769;
  const Iz = data.Iz * 1e4, It = data.It * 1e4, Iw = data.Iw * 1e6;
  const { kz, kw } = getTorsionWarping();
  const { C1, C2, C3 } = getCoefficients();
  const Lmm = L * 1000;
  
  const termA = (Math.PI ** 2 * E * Iz) / ((kz * Lmm) ** 2);
  const termB = ((kz / kw) ** 2) * (Iw / Iz);
  const termC = (((kz * Lmm) / Math.PI) ** 2) * (G * It) / (E * Iz);
...
function calculateMcr() {
  ...
  const data = profilesDB[profile];
  if(!data) return 0;
  const { kz } = getTorsionWarping();
  if(document.getElementById('kCoeff')) document.getElementById('kCoeff').innerHTML = kz.toFixed(2);
  return calculateMcrParams(L, support, zg, zj, null, data);
}
...
function compareProfiles() {
  ...
  Object.keys(profilesDB).forEach(key => {
    ...
    const betaW = getBetaW(sectionClass, data);
    const Mcr = calculateMcrParams(L, support, zg, zj, null, data);
    const lambdaLT = Math.sqrt((data.Wpl * 1000 * fy * betaW) / (Mcr * 1e6));
    const phiLT = 0.5 * (1 + alphaLT * (lambdaLT - 0.2) + lambdaLT * lambdaLT);
...
function switchMobileTab(tab) {
  ...
  if(tab === 'input') {
    ...
    // Ensure diagrams are drawn/resized when the tab becomes visible
    drawDiagrams();
    drawDeflectionDiagram();
    drawSupportDiagram();
    if(momentChart) momentChart.resize();
    if(shearChart) shearChart.resize();
    if(deflectionChart) deflectionChart.resize();
  }
}
...
function initScrollReveal() {
  ...
  // Safety net: reveal everything after 300ms no matter what
